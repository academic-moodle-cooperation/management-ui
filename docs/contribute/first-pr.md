...
A working checkout first: [Set up the repo](./setup.md). New to the codebase? [Architecture](../reference/architecture.md) is the tour, [`AGENTS.md`](../../AGENTS.md) the operational rules — written for AI agents, equally useful as a human pre-flight checklist.
...
- **What CI runs.** The [docs fast path](./ci.md#changes-that-skip-ci-gates), not the full gate. Docs need no changeset — but a README *inside* `packages/*` or `plugins/*` is part of a versioned package and does.
