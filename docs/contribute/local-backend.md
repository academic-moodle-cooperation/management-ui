...
For contributors who need real data locally. Afterwards you'll have an Opencast running on your machine with the Management UI bundles deployed.
...
[Run from source → Configure the backend](./setup.md#pick-a-backend).
...
> [operations/open-followups.md](https://github.com/academic-moodle-cooperation/management-ui/blob/develop/docs/reference/open-followups.md).

## Prerequisites

- git, **JDK 21**, **Maven ≥ 3.6** — `mvn -version` must report Java 21
- **Node** and **pnpm** at the versions pinned in the root [`package.json`](../../package.json) (`engines` and `packageManager`) — `corepack enable` activates the right pnpm automatically
...
[Configure](../operate/configure.md#the-file)).
...
- **Your own plugin, hot-reloading:** scaffold with `pnpm create-plugin
  ...
  automatically. See [Creating a plugin](../extend/plugin-guide.md).
- **Org/community plugins as JARs:** drop the plugin JAR into the same
  ...
  it from `plugins.json`. Full details: [Distribution](../extend/distribution.md).
