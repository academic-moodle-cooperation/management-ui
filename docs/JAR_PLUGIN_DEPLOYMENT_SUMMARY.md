...
```
1. Build frontend (dist/my-plugin.mjs + my-plugin.css)
2. Build backend JAR (Maven copies dist/ into JAR, sets OSGi headers)
3. Deploy JAR to Opencast deploy/
4. Backend scans JARs for Management-Plugin header → builds plugins.json
5. Frontend fetches plugins.json → loads each plugin via RemoteLoader
```

## 1. JAR generation (build)

- **Frontend:** In the plugin repo you run `pnpm build`. Output: `dist/my-plugin.mjs`, `dist/my-plugin.css`.
...
- **Output:** One JAR (e.g. `my-org-plugin-backend-1.0-SNAPSHOT.jar`) containing Java code (if any) and `static/plugins/<plugin-id>/my-plugin.mjs` + `my-plugin.css`.
...
- For each such bundle it builds a **PluginConfig**: `name` (bundle symbolic name), `path` = `/static/plugins/<Management-Plugin value>`, `scope` = `management_ui_plugin_<sanitized>`.
- **PluginEndpoint** exposes `GET /management-tool/ui/config/plugins.json`, which returns `{ "plugins": [ { "name", "path", "scope" } ] }`.
...
- **Core** (not the marketplace) loads JAR plugins. In `apps/management-ui-core`, **PluginInitializer** calls `loadJarPlugins()` from `src/services/jarPluginLoader.ts`:
  ...
  - For each entry, builds the plugin script URL: `base + path + "/" + <last path segment> + ".mjs"` (e.g. `/management-ui/static/plugins/my-org/my-org.mjs`).
