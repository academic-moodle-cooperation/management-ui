...
| 9.4 | `pnpm dev` and reload | Browser console shows `[INFO] [demo-local] activated` (logged via `logger.info`, visible at DevTools' default level — no need to enable "Verbose"). The plugin's logo placeholder also appears in the header (the `/assets/demo-local-logo.svg` image 404s — that's the expected placeholder; the registration taking effect is the real signal). **If you see neither:** `demo-local` isn't in the *served* config's `app.enabledPlugins`, so the plugin never activates — re-check §9.3 (running without a backend, or `VITE_LOCAL_CONFIG=true`). Activation only runs for plugins the served config enables. |
