...
3. **Resolve version metadata toward the newer line.** Forward merges conflict in `package.json` versions, `CHANGELOG.md`s, and `VERSION`: always keep the **target (newer) line's** values and take only the fix itself. `VERSION` is per-line by design (`18.5.1` on `r/18.x`, `19.2.0` on `r/19.x`) and is bumped **automatically** inside the Version PR: `pnpm changeset:version` runs `scripts/bump-product-version.mjs`, which keeps the major pinned to the line's Opencast major and derives minor/patch from the npm bumps the Version PR contains.
...
5. **Only the newest line publishes npm.** `release.yml` computes the newest existing `r/NN.x` at run time: the newest line runs `changeset publish` (npm + per-package tags, tagged `latest` — correct by construction); an **older line never touches npm** — its release leg runs `changeset tag` and still produces the per-package git tags, the product tag and the GitHub Release. Older lines ship as JARs. No manual `npm dist-tag` repointing exists anymore.
