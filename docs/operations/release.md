...
> - **Secret `NPM_TOKEN`:** an npm automation token with publish rights, added under *Settings → Secrets and variables → Actions*. It's read by the workflow but only used once packages are public.
