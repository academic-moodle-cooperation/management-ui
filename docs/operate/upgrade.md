...
The JAR filenames answer it:

```bash
ls "$OPENCAST_HOME"/deploy/management-ui-*.jar
```

Released artifacts carry the **product version** in the name: the current line's `VERSION` is `20.0.0`, so its bundles are `management-ui-core-20.0.0.jar` and its two siblings. A build from an unreleased checkout carries Maven's snapshot version (`1.0-SNAPSHOT`) instead — then "your version" is whichever commit you built, and only your own build records say which one.
