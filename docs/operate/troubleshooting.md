...
| A deployed `management-ui-*` bundle stays unresolved, with `missing requirement … osgi.wiring.package=…` and a version range | The JAR was built against a different Opencast major than the one it runs on — usually because the build ran on a fresh clone's default branch instead of the matching release line. OSGi import ranges are fixed at compile time: check out `r/<your major>.x` and rebuild ([Install → Build and deploy](./install.md#build-and-deploy)) |
...
| You set one key in `org.opencastproject.mui.cfg` and other behaviour changed too | A `.cfg` replaces the PID's **whole** dictionary; keys you omitted fell back to their metatype defaults, and `trash.workflow.id` has none. Always write all six keys ([Backend configuration](./backend-config.md#set-it-explicitly)) |
| You deleted `org.opencastproject.mui.cfg` to get the shipped values back, and did not | The bundle's configurator resource is applied once and never re-applied, so removing the file removes the values instead of restoring them. Reinstall the bundle — touching the JAR in `deploy/` is enough ([Backend configuration](./backend-config.md#applying-a-change)) |
...
| Deleting a video does nothing: red toast *"The video could not be moved to the trash!"*, GraphQL says `Workflow definition '<org>/<id>' not found or inaccessible` | `trash.workflow.id` names a workflow the server does not have (an empty value counts) — the default state on any Opencast where no trash workflow was installed. Check with `curl -s -u <admin-user> "https://opencast.example.org/workflow/definitions.json"`, then install that workflow or unset the key deliberately ([Backend configuration](./backend-config.md#set-it-explicitly)) |
