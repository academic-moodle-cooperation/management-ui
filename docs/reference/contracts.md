...
> **Reference — look it up, don't read it through.** This page is the frozen list of what a plugin may rely on, consulted a section at a time. Task-shaped guidance lives under [Extend](../extend/index.md) and [Contribute](../contribute/index.md).
...
4. [Config Contract](#4-config-contract) - how plugins declare and consume configuration (full model: [Configuration model](./configuration.md)).
...
The Manifest 1.1 and Runtime API 1.1 contracts are **mechanically verified** by the contract-test harness in [`@oc-mui/plugin-testing`](../../packages/plugin-testing/README.md); see [`docs/contribute/testing.md`](../contribute/testing.md) for the test pyramid and harness usage.
...
**Authoritative document:** [`plugins/styling.md`](../extend/styling.md).
...
**Authoritative spec:** [Configuration model](./configuration.md) — full layer model, loader phases, and the `definePluginConfig` / `useConfig` reader API.
...
- **JAR** — the shell applies the same gate to entries from the backend's `plugins.json`. The gate is a no-op until the backend includes `workspaceDependencies` per plugin in `plugins.json` (the shell types are forward-compatible and read it when present); that backend step is tracked in [`operations/open-followups.md` §5.3](https://github.com/academic-moodle-cooperation/management-ui/blob/develop/docs/reference/open-followups.md#53-shared-npm-deps-version-locking).
...
3. If it is a major bump, an ADR explaining the break and a migration note in the affected package's `CHANGELOG.md` (the policy: [Releases & versioning → Deprecations](../contribute/release.md#deprecations)).
...
- **2026-04-17:** Config Contract promoted to 1.0. Stable keys: `app.theme`, `app.locale`, `app.enabledPlugins`, `config.plugins[pluginId]`, `config.plugins[pluginId].enabled`. Layered merge order (`app:config:defaults` ⊕ base ⊕ `app:config`) and the `definePluginConfig` reader API are now frozen for 1.x. See [Configuration model](./configuration.md).
...
- **2026-05-13:** Shared Runtime Dependencies Contract 1.0 added. New section [§5](#5-shared-runtime-dependencies) formalises the list of host-provided packages (`SHARED_RUNTIME_MAJORS` in `@oc-mui/plugin-system`) and the rule that a plugin's `workspaceDependencies` lower-bound major must match the host's. New helpers `checkSharedDependencyCompatibility` and `parseRangeMajor` are exported from `@oc-mui/plugin-system`. The runtime check is **not yet wired** into the JAR loader or `.local-plugins/` discovery path — those follow-ups are tracked in [Open follow-ups](https://github.com/academic-moodle-cooperation/management-ui/blob/develop/docs/reference/open-followups.md#53-shared-npm-deps-version-locking). Existing plugins are unaffected; the contract is additive. *(Superseded: the check is now wired into all three loader paths — see [§5](#5-shared-runtime-dependencies).)*
- **2026-05-14:** GraphQL Operation Naming Contract 1.0 added. New section [§6](#6-graphql-operation-naming) mandates that every `query`/`mutation`/`subscription`/`fragment` declared in a plugin (or in `@oc-mui/query` for shared core operations) be prefixed with the plugin's namespace in PascalCase (`MuiGetMyEvents`, `EpisodesEpisodeFields`, …). Shipping with documentation + manual review only; an ESLint rule that fails CI on violations is the next planned follow-up. Existing operations in `packages/query/src/queries.graphql` are grandfathered and will be renamed in a follow-up batch — see [Open follow-ups](https://github.com/academic-moodle-cooperation/management-ui/blob/develop/docs/reference/open-followups.md#51-graphql-operation-naming-final-migration-of-legacy-operations). *(Superseded by the two 2026-05-15 entries below: the ESLint rule shipped and the legacy operations were renamed.)*
