...
Existing operations in [`packages/query/src/queries.graphql`](../../packages/query/src/queries.graphql) (which today use bare names like `GetMyEvents` plus a partial `PluginXxxFields` pattern) are temporarily **grandfathered** through `# eslint-disable-next-line local/graphql-operation-naming` comments. The disable comments double as the migration tracker — grep for them to see what's left to rename. The actual rename batch is tracked in [`operations/open-followups.md`](../operations/open-followups.md#51-graphql-operation-naming-final-migration-of-legacy-operations).
