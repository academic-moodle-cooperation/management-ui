...
For plugin authors shipping to a deployment. Afterwards you'll know the four delivery paths a plugin can take from your editor to a running shell, and which one fits your case.

Reference — pick the path that matches how your plugin will be operated.
...
Each `.local-plugins/<name>/` is its own git repo, gitignored from this monorepo. `pnpm-workspace.yaml` lists `.local-plugins/*` as a workspace glob, so `@oc-mui/*` dependencies resolve normally.

Workflow:

```bash
# Mount it, then re-run install from the workspace ROOT so pnpm picks up the
# new workspace member — without this the plugin has no node_modules and
# `@oc-mui/*` won't resolve.
git clone <your-plugin-repo> .local-plugins/my-plugin
pnpm install
...
The backend tracks every OSGi bundle carrying the `Management-Plugin` header. For each, it reads `static/plugins/<plugin-id>/plugin.json` (the canonical source of truth) and emits one entry per module declared in the manifest. If `plugin.json` is missing or unparseable, the tracker falls back to filename-convention discovery: every `*.mjs` in the static directory becomes its own entry. Both strategies, their precedence, and which headers each one reads: [Backend bundles](./backend-bundles.md#two-discovery-strategies-and-which-wins).
