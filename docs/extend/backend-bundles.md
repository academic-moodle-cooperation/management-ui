...
Two details worth knowing before you edit either file. `trash_workflow_id()` is the one method **without** a Java `default` — it is only ever set by the configurator resource, so a deployment that removes it from `mui.json` gets `null` and permanent deletes ([#342](https://github.com/academic-moodle-cooperation/management-ui/issues/342)). And the resource's `:configurator:symbolic-name` says `management-ui-graphql` — the bundle that ships the file, matching the OSGi Configurator's own default for in-bundle resources (it used to carry an unrelated Java package name; corrected in [#352](https://github.com/academic-moodle-cooperation/management-ui/issues/352)).
