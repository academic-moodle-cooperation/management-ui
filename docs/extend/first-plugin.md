...
`--template app` gives you a **real screen plus a sidebar link**, visible in both dev and production. The default (`--template minimal`) registers an `app:header-logo` placeholder that nothing in the core shell renders — the contract test passes, but there's nothing to see.
...
`src/index.ts` registers exactly two things in `initialize()` — the screen and the link to it:
...
Keep the `export default` — the loader registers via `module.default`.
...
Green out of the box, checking: the plugin activates, every `extensionPoints` entry in `plugin.json` is populated, there's a `default` export (this assertion is written into the scaffolded test itself — the harness doesn't check it), no console errors, and i18n key parity (a no-op until you ship `locales/`).

## Next

- **Make it real** — [Building a plugin](./plugin-guide.md) continues with this same `hello` plugin: a config slice your deployment can override, a translated string, and the rest of the extension points.
