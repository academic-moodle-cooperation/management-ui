...
|---------|------------------|
| `@oc-mui/tailwind-config` | Full default config — preset + dark mode (class strategy) + container utilities + animations. |
| `@oc-mui/tailwind-config/preset` | The shadcn preset on its own. Use this when you want to compose with your own config. |
| `@oc-mui/tailwind-config/plugin` | The underlying Tailwind plugin (CSS-variable colour system, container utilities, typography extensions). |
...
- **Animations** — Tailwind's defaults plus `tailwindcss-animate` and a `bouncing-loader` keyframe used by `@oc-mui/ui`'s loaders.
