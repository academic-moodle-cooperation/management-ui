...
| `definePluginConfig({ id, schema, defaults })` | Builds a typed config reader. Returns `{ id, schema, defaults, register, use, read }`. Reading another plugin's slice is forbidden — enforced by the `local/no-cross-plugin-config` lint rule. |
