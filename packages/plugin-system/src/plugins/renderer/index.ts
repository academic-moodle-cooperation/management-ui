      ...
      manager.addFunction('renderer.add', (position: string, component: React.FC, key = crypto.randomUUID(), order = 100) => {
        ...
        try {
          ...
          if (positionComponents.length > 1) {
            const orders = positionComponents.map(c => (typeof c.order === 'number' ? c.order : 100));
            const minOrder = Math.min(...orders);
            const topComponents = positionComponents.filter(c => (typeof c.order === 'number' ? c.order : 100) === minOrder);
            if (topComponents.length > 1 && position.startsWith('component-override:')) {
              const keys = topComponents.map(c => c.key).join(', ');
              console.warn(`[plugin-system][renderer] Multiple components registered with the same priority (order ${minOrder}) for "${position}". The first registered will be used. Conflicting keys: ${keys}`);
            }
          }
        } catch (e) {
          console.error('[plugin-system][renderer] Failed to evaluate component priorities for warning:', e);
        }
