...
export const useRendererStore = () => {
  ...
  const addComponent = useCallback((position: string, component: React.FC, key = crypto.randomUUID()) => {
    setComponents(prev => {
  ...
  const removeComponent = useCallback((position: string, key: string) => {
    setComponents(prev => {
      const newMap = new Map(prev);
      const existing = newMap.get(position) || [];
      newMap.set(position, existing.filter(c => c.key !== key));
      manager.dispatchEvent('renderer.componentUpdated', { position });
      return newMap;
    });
  }, [manager]);

  const getComponents = useCallback((position: string) => {
