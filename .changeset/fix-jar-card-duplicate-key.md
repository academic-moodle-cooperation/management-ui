...
A single JAR can ship multiple modules under one `scope` (e.g. the
University of Vienna JAR registers `app`, `sidebar`, `footer`, and
`landing-page` all under scope `management_ui_plugin_univie`). The card
...
The per-module `id` (e.g. `univie/app`) is the only field the backend
