...
`@opencast-mui/eslint-config` and `@opencast-mui/tailwind-config` had no `files` field,
...
`@opencast-mui/typescript-config` already had `files: ["*.json"]` and is unchanged.
...
`@opencast-mui/eslint-config` additionally gets its dependencies fixed: the ESLint
