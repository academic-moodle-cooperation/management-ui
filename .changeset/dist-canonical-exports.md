...
Now the top-level `exports` are `dist`-canonical for consumers, with a
`development` condition pointing at `src` so the dev server and tests still resolve
source with no build step. `publishConfig` is reduced to `{ "access": "public" }`.
No public API changed; this is packaging metadata only.
