---
"@opencast-mui/plugin-admin-marketplace": minor
---
...
1. **Marketplace `RemoteLoader` now enforces `apiVersion`.** When a registry
   ...
   `checkApiVersionCompatibility` from `@opencast-mui/plugin-system` before any
...
2. **`@opencast-mui/*` workspace packages bumped from `0.0.0` to `1.0.0`** to
