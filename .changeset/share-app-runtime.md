...
"@opencast-mui/remote-plugin-loader": minor
---

Expose `@opencast-mui/app-runtime` as a host-provided shared module so app-type
...
- The remote-plugin loader now logs a clear, actionable error naming any
  `@opencast-mui/*` package a plugin imports that the host doesn't expose — instead of
...
- `checkSharedDependencyCompatibility` now **rejects** a wrong-scope reference to
  ...
  pre-rename namespace, while the host provides `@opencast-mui/plugin-system`) with a
