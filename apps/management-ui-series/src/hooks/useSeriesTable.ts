...
export function useSeriesTable() {
  ...
  const { data: seriesInputFields, isLoading: isLoadingMetadata } = useGetSeriesByIdInputFieldsQuery(
    { seriesId: selectedId },
  ...
  const setSeriesUpdateData = useCallback((data: SeriesUpdateData | undefined) =>
    dispatch({ type: 'SET_SERIES_UPDATE_DATA', payload: data }), [dispatch]);
