...
import { exampleHeaderLogoPlugin } from "./modules/header-logo-example";

// The example plugin keeps its entry at the package root (no `src/`), so
// the plugin directory is one level up from this test file.
...
describe("example plugin contract", () => {
  ...
  beforeAll(async () => {
    harness = await loadPluginInHarness(exampleHeaderLogoPlugin, {
